...
SELECT *
FROM building
WHERE cost > ( SELECT MAX(cost) FROM building WHERE city = 'can tho' );
...
-- 04. Hiển thị thông tin công trình chưa có KTS thiết kế
SELECT * FROM building
WHERE id NOT IN (SELECT building_id FROM design);

SELECT * FROM building
WHERE id IN (SELECT building_id FROM design);

-- 05. Hiển thị thông tin các KTS cùng năm sinh và cùng nơi tốt nghiệp
