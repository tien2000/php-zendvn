...
class Validate{
            ...
            if ($value['required'] == true && trim($this->_source[$elemt] == null)) {                
                ...
                switch ($value['type']) {
                    ...
                    case 'existRecord':         
                        ...
                        break;
                    case 'file':         
                        $this->validateFile($elemt, $value['options']);
                        break;
                }
            }            
            if (!array_key_exists($elemt, $this->_errors)) {
    ...
    public function validateFile($elemt, $options){
        if (!filter_var($this->_source[$elemt]['size'], FILTER_VALIDATE_INT, array('options' => array('min_range' => $options['min'], 'max_range' => $options['max'])))) {               
            $this->setError($elemt, 'Invalid Size');       
        }

        $ext = pathinfo($this->_source[$elemt]['name'], PATHINFO_EXTENSION);

        if (in_array($ext, $options['extension']) == false) {
            $this->setError($elemt, 'Invalid Extension');
        } else {
            # code...
        }
